...
func @matmul_call(%A: memref<60x80xi32>, %B: memref<80x72xi32>, %C: memref<60x72xi32>) {
  linalg.matmul {__internal_linalg_transform__="Ignore"}
   ins(%A, %B: memref<60x80xi32>, memref<80x72xi32>)
   outs(%C: memref<60x72xi32>)
  return
}
...
func @main() {
  ...
  %c0 = arith.constant 0 : index
  %c8 = arith.constant 80 : index
  %c16 = arith.constant 60 : index
  %c32 = arith.constant 72 : index
  ...
  %A_typed = memref.cast %A: memref<?x?xi32> to memref<60x80xi32>
  %B_typed = memref.cast %B: memref<?x?xi32> to memref<80x72xi32>
  %C_typed = memref.cast %C: memref<?x?xi32> to memref<60x72xi32>
  %Ctmp_typed = memref.cast %Ctmp: memref<?x?xi32> to memref<60x72xi32>
  
  %in1 = memref.cast %A_typed: memref<60x80xi32> to memref<*xi32>
  %in2 = memref.cast %B_typed: memref<80x72xi32> to memref<*xi32>
  %out1 = memref.cast %C_typed: memref<60x72xi32> to memref<*xi32>
  %outtmp = memref.cast %Ctmp_typed: memref<60x72xi32> to memref<*xi32>

  call @print_memref_i32(%in1) : (memref<*xi32>) -> ()
  call @print_memref_i32(%in2) : (memref<*xi32>) -> ()

  call @matmul_call(%A_typed, %B_typed, %C_typed) : (memref<60x80xi32>, memref<80x72xi32>, memref<60x72xi32>) ->()
