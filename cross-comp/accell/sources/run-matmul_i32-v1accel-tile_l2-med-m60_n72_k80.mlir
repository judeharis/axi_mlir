...
func @matmul_call(%A: memref<200x240xi32>, %B: memref<240x220xi32>, %C: memref<200x220xi32>) {
  linalg.matmul {__internal_linalg_transform__="MEM"}
   ins(%A, %B: memref<200x240xi32>, memref<240x220xi32>)
   outs(%C: memref<200x220xi32>)
  return
}
...
func @main() {
  ...
  %c0 = arith.constant 0 : index
  %c8 = arith.constant 240 : index
  %c16 = arith.constant 200 : index
  %c32 = arith.constant 220 : index
  ...
  %A_typed = memref.cast %A: memref<?x?xi32> to memref<200x240xi32>
  %B_typed = memref.cast %B: memref<?x?xi32> to memref<240x220xi32>
  %C_typed = memref.cast %C: memref<?x?xi32> to memref<200x220xi32>
  %Ctmp_typed = memref.cast %Ctmp: memref<?x?xi32> to memref<200x220xi32>
  
  %in1 = memref.cast %A_typed: memref<200x240xi32> to memref<*xi32>
  %in2 = memref.cast %B_typed: memref<240x220xi32> to memref<*xi32>
  %out1 = memref.cast %C_typed: memref<200x220xi32> to memref<*xi32>
  %outtmp = memref.cast %Ctmp_typed: memref<200x220xi32> to memref<*xi32>

  call @print_memref_i32(%in1) : (memref<*xi32>) -> ()
  call @print_memref_i32(%in2) : (memref<*xi32>) -> ()

  call @matmul_call(%A_typed, %B_typed, %C_typed) : (memref<200x240xi32>, memref<240x220xi32>, memref<200x220xi32>) ->()
