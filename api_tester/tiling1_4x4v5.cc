// Example tiling code which uses MM_4x4v5 accelerator.
// Example takes advantage of C stationary capability of the accelerator to
// reduce the number of times C data is sent back from the accelerator

#include "mlir/ExecutionEngine/axi/api_v1.h" // Requires #define ACC_V2 to use the correct accelerator --- "-DACC_V5"
#include "mm_helper.h"
#include <cstdlib>
#include <iomanip>
#include <iostream>
...
int main(int argc, char *argv[]) {
  ...
  for (int m = 0; m < pM; m += tile_M) {
    for (int n = 0; n < pN; n += tile_N) {
      ...
      uint32_t h = 1;
      dma_inbuffer[0] = h;
      // Sends opcode to Store and waits
      ...
      // Gets pointer to DMA_OUT_BUFFER and C_Base
