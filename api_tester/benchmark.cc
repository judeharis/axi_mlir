...
#define depth 4
    ...
    for (int i=0;i<1000;i++){
        ...
        dma1.dma_wait_send();
        dma1.dma_wait_recv();
        dma1.dma_copy_from_outbuffer(reinterpret_cast<unsigned int*>(accelerated_outputs+ i*16),cols*rows,i*16);
    }
    prf_end(1,duration);
    double fduration = std::chrono::duration<double, std::milli>(duration).count();
    ...
    outfile << "";
    outfile.close();
    for (int i=0;i<10;i++)save_matrix("out.txt",rows,cols,accelerated_outputs + i*16);
