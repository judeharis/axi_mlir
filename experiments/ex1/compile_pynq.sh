...
# debug: 0*, 1
# test: 0*, 1
#example: ./compile.sh # default target is arm, debug is 0  and test is 0
#example: ./compile.sh arm 1 0
#example: ./compile.sh sysc 1 0 
...
source scripts/compile_mlir_matmul-all-ir.sh
...
# echo "declare -a FlowArray=(" >>$OUTDIR/appslist.sh
# for FLOW in ${FlowArray[@]}; do
#   echo "    ${FLOW}" >>$OUTDIR/appslist.sh
# done
# echo ")" >> $OUTDIR/appslist.sh

# echo "declare -a KernelNameArray=(" >>$OUTDIR/appslist.sh
# for KERNEL_NAME in ${KernelNameArray[@]}; do
#   echo "    ${KERNEL_NAME}" >>$OUTDIR/appslist.sh
# done
# echo ")" >> $OUTDIR/appslist.sh


echo "declare -a AccelSizeArray=(" >>$OUTDIR/appslist.sh
for ACCEL_SIZE in ${AccelSizeArray[@]}; do
  echo "    ${ACCEL_SIZE}" >>$OUTDIR/appslist.sh
done
echo ")" >> $OUTDIR/appslist.sh


echo "declare -a ProblemDimArray=(" >>$OUTDIR/appslist.sh
for PROBLEM_DIM in ${ProblemDimArray[@]}; do
  echo "    ${PROBLEM_DIM}" >>$OUTDIR/appslist.sh
done
echo ")" >> $OUTDIR/appslist.sh

echo "declare -a StrategyArray=(" >>$OUTDIR/appslist.sh
for STRATEGY in ${StrategyArray[@]}; do
  echo "    ${STRATEGY}" >>$OUTDIR/appslist.sh
done
echo ")" >> $OUTDIR/appslist.sh

echo "declare -a AccelTypeArray=(" >>$OUTDIR/appslist.sh
for ACCEL_TYPE in ${AccelTypeArray[@]}; do
  echo "    ${ACCEL_TYPE}" >>$OUTDIR/appslist.sh
done
echo ")" >> $OUTDIR/appslist.sh

echo "declare -a AppNameArray=(" >>$OUTDIR/appslist.sh
...
# output an sh array with the apps name
...
if [ $TARGET == "sysc" ]; then
  ...
  echo "export LD_LIBRARY_PATH=$PROJ_ROOT/experiments/ex1/ex1_sysc:/working_dir/builds/llvm-project/build-x86/lib"
fi
